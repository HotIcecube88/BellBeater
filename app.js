...
function mergedTemplateBlocks(isFriday){
  ...
  if (prefs.double34) {
    const i3 = out.findIndex(b=>b.id==='3');
    const iL = out.findIndex(b=>b.isLunch && (b.id==='4A' || b.id==='4B' || b.id==='5C'));
    const i4 = out.findIndex(b=>b.id==='4');
    if (i3>=0 && i4>=0 && (iL<0 || iL<i3 || iL>i4)) {
      const b3 = out[i3], b4m = out[i4];
      const merged = { id:'3_4', name: labelFor('3') + ' + ' + labelFor('4'), start: b3.start, end: b4m.end, isLunch:false };
      out.splice(i3, i4-i3+1, merged);
    }
  }
...
function startLoop(){ clearInterval(tick); updateNow(); tick=setInterval(updateNow,1000); }
startLoop();
...
function updateNow(){
  ...
  if (!blocks.length){
    ...
    $('#wCountdown')?.textContent='--:--'; $('#wSub')?.textContent='—'; progressBar.style.width='0%';
    return;
  }
  ...
  const wCount = $('#wCountdown'), wSub=$('#wSub');
  if(wCount && wSub){
...
function maybeNotifyWarnings(current, now){
  ...
  if (prefs.pre5 && now.getTime() >= fiveMs && notifState.pre5Sent !== endMs) {
    notifState.pre5Sent = endMs;
    showNotif(`${current.name} ends in 5 minutes`);
  }
  if (prefs.pre1 && now.getTime() >= oneMs && notifState.pre1Sent !== endMs) {
    notifState.pre1Sent = endMs;
