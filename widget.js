...
const prefs = loadJSON(SKEY_PREFS, {});
...
function mergedTemplateBlocks(isFriday){
  ...
  const L = prefs.lunch;
  const out=[]; const push=(id,name,start,end,isLunch=false)=>out.push({ id,name,start,end,isLunch });
  ...
  else { ['4A','4','4B','5','5C'].forEach(id=>{ const b=get(id); if(b) push(b.id, labelFor(id), b.start,b.end,false); }); }
  ['6','7'].forEach(id=>{ const b=get(id); if(b) push(id, labelFor(id), b.start,b.end,false); });
  return out;
}
function blocksForToday(){ const isFri=(new Date().getDay()===5); return mergedTemplateBlocks(isFri); }
function update(){
  const now = new Date();
  ...
  const cnt=document.getElementById('cnt'); const sub=document.getElementById('sub');
  if(current){ const left=current.endD-now; cnt.textContent = durFmt(left); sub.textContent='Ends '+fmtHM(current.endD); }
  else if(next){ const until=next.startD-now; cnt.textContent=durFmt(until); sub.textContent='Next '+next.name+' @ '+fmtHM(next.startD); }
  else { cnt.textContent='--:--'; sub.textContent='—'; }
}
update();
